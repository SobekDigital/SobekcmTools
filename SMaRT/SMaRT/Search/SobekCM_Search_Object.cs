...
namespace SobekCM.Management_Tool.Search
{
    ...
    public class SobekCM_Search_Object : IEquatable<SobekCM_Search_Object>
    {
        ...
        public DataSet Perform_Tracking_Search()
        {
            if ((First_Value.Length == 0) && (Second_Value.Length == 0) && ( Third_Value.Length == 0) && ( Fourth_Value.Length == 0))
            {
                ...
                if (code.Length > 0)
                {
                    try
                    {
                        DataSet dataSet = Engine_Database.Tracking_Get_Item_Aggregation_Browse(code);
                        return dataSet;
                    }
                    catch
                    {
        ...
        private DataSet Perform_Database_Search()
        {
            ...
            ResultsServices.Split_Clean_Search_Terms_Fields(searchString, fieldString, Search_Type_Enum.Advanced, terms, web_fields, null, Search_Precision_Type_Enum.Contains, '|');
            ...
            if (actualCount == 1)
            {
                ...
                if ((web_fields[0] == "OC") && (terms[0].Length > 0))
                {
                    ...
                    if (is_number)
                    {
                        ...
                        return Engine_Database.Tracking_Items_By_OCLC_Number(oclc, null);
                    }
                }
                ...
                if ((web_fields[0] == "AL") && (terms[0].Length > 0))
                {
                    ...
                    if (is_number)
                    {
                        ...
                        return Engine_Database.Tracking_Items_By_ALEPH_Number(aleph, null);
                    }
                }
            }
            ...
            if (Search_Precision == Search_Precision_Type_Enum.Exact_Match)
            {
                return Engine_Database.Tracking_Metadata_Exact_Search(terms[0], db_fields[0], code);
            }
            ...
            if (simplified_search)
            {
                ...
                return Engine_Database.Tracking_Metadata_Search(searchBuilder.ToString(), code);
                // OLD CODE WHEN USING THE SIMPLE METADATA SEARCH WHICH INCLUDES THE SAME LINK TYPE
                //return Database.Engine_Database.Perform_Metadata_Search(terms[0], terms[1], terms[2], terms[3],
                //  terms[4], terms[5], terms[6], terms[7], terms[8], terms[9], main_link, include_private, Current_Mode.Aggregation, Tracer);

            }
                
            return Engine_Database.Tracking_Metadata_Search(terms[0], db_fields[0], links[0], terms[1], db_fields[1], links[1], terms[2], db_fields[2], links[2], terms[3],
